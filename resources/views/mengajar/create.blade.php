...
@section('content')
    <div class="container-form">
        <h2 align="center">Add Teaching</h2>

        <x-alert />

        <form action="{{ route('teachings.store') }}" method="POST" autocomplete="off">
            @csrf
            ...
            <select name="teacher_id" id="teacher_id" autofocus>
                ...
                @foreach ($teachers as $teacher)
                    <option value="{{ $teacher->id }}" {{ old('teacher_id') == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }} {{ $teacher->nip }}</option>
                @endforeach
             </select>
            @error('teacher_id')
            ...
            <select name="subject_id" id="subject_id">
                ...
                @foreach ($subjects as $subject)
                    <option value="{{ $subject->id }}" {{ old('subject_id') == $subject->id ? 'selected' : '' }}>{{ $subject->label }}</option>
                @endforeach
             </select>
            @error('subject_id')
                <br><span style="color: red">{{ $message }}</span>
